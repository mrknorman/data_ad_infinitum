...
class PredictionPlotter(tf.keras.callbacks.Callback):
    def __init__(self, tf_dataset, output_name="amplitude", filename="model_predictions"):
        super().__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        batch = next(iter(self.tf_dataset))
        
        for example in batch:
            onsource = tf.convert_to_tensor(example['onsource'])
            ground_truth = example[self.output_name].numpy()

            predictions_distribution = self.model(onsource)

            predictions_loc = predictions_distribution.mean().numpy()
            predictions_scale = predictions_distribution.stddev().numpy()

            x = np.linspace(0.0, max(ground_truth), 1000)

            output_file(f"{self.filename}_{i}.html")

            p = figure(width=800, height=400, title='Predictions vs Ground Truth')

            vline = Span(location=ground_truth[i], dimension='height', line_color='green', line_width=2)
            p.renderers.extend([vline])

            vline = Span(location=predictions_loc[i][0], dimension='height', line_color='blue', line_width=1)
            p.renderers.extend([vline])

            y = (1/(predictions_scale[i] * np.sqrt(2 * np.pi))) * np.exp(-0.5*((x - predictions_loc[i][0]) / predictions_scale[i][0])**2)
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        ...
        def transform_features_labels(features, labels):
            labels[output_name] = labels[output_name][0]
            return features, labels
        ...
        plot_ds = get_ifo_data_generator(
            time_interval = O3,
            data_labels = ["noise", "glitches", "events"],
            ifo = 'L1',
            injection_configs = injection_configs,
        ...
        callbacks=[PredictionPlotter(plot_ds, output_name=output_name, filename="model_predictions")]
        
        builder.train_model(cbc_ds, num_train_examples//num_examples_per_batch, 10, callbacks = callbacks)

...
def pathfind(
        ...
        restart_count : int = 0,
        # Model Arguments:
        num_validate_examples : int = int(2E6),
        # Dataset Arguments: 
    ...
    data_directory_path = current_dir / "validation_noise_files"
    ...
    validate_dataset : tf.data.Dataset = gf.Dataset(
        **deepcopy(dataset_arguments),
        group="validate"
    ).take(num_validate_examples // gf.Defaults.num_examples_per_batch)

    for i in tqdm(validate_dataset):
        pass

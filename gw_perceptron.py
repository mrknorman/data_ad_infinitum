...
from py_ml_tools.model   import ModelBuilder
...
if __name__ == "__main__":
    ...
    print(json.dumps(injection_configs, indent=4))
    ...
    train_dataset = \
        group_split_dataset(generator_args, "train", num_train_examples).map(adjust_features)
    
    validate_dataset = \
        group_split_dataset(generator_args, "validate", num_validate_examples).map(adjust_features)

    test_dataset = \
        group_split_dataset(generator_args, "test", num_test_examples).map(adjust_features)
